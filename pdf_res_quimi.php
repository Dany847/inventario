<?php
	include 'plantilla_res.php';
	require 'connect.php';
	 $consulta = $conn->query("select * from residuos  where idarea = '3'
	  ") or die(mysqli_error());
	$pdf = new PDF();
	$pdf->AliasNbPages();
	$pdf->AddPage();
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(215,6, utf8_decode('LABORATORIO DE PESADO'),0,0,'C' );
	$pdf->Ln(10);
	$pdf->SetFillColor(255,255,255);
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(185,6, utf8_decode('M.C. Federico Francisco Martínez'),0,0,'C' );
	$pdf->Ln(5);
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','',12);
	...
	$pdf->Cell(20,6, utf8_decode('UNIDAD'),1,0,'C',1);
	$pdf->Cell(40,6, utf8_decode('CLASIFICACIÓN'),1,0,'C',1);
	$pdf->Cell(40,6, utf8_decode('RECIPIENTE'),1,0,'C',1);
	$pdf->Cell(50,6, utf8_decode('OBSERVACIONES'),1,1,'C',1);
	$pdf->SetFont('Arial','',10);
	while($fila = $consulta->fetch_array())
	{
		$pdf->Cell(10,6, utf8_decode(''),0,0,'C',1);
		$pdf->Cell(25,6,utf8_decode($fila['cantidad']),1,0,'C');
		$pdf->Cell(20,6,utf8_decode($fila['unidad']),1,0,'C');
		$pdf->Cell(40,6,utf8_decode($fila['clasificacion']),1,0,'C');
		$pdf->Cell(40,6,utf8_decode($fila['recipiente']),1,0,'C');
		$pdf->Cell(50,6,utf8_decode($fila['observaciones']),1,1,'C');
	}
	$pdf->Ln(6);
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','B',10);
...
<?php
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(190,6, utf8_decode('Ing. Antonio Gómez Salazar'),0,0,'C' );
	$pdf->Ln(5);
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(39,6, utf8_decode('Periodo: '),0,0,'C' );
