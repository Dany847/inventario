<?php
	include 'plantilla_m.php';
	require 'connect.php';
	 $consulta = $conn->query("select * from reactivos  where idarea = '3'
	  ") or die(mysqli_error());
	$pdf = new PDF();
	$pdf->AliasNbPages();
	$pdf->AddPage();
	...
	$pdf->Cell(25,6, utf8_decode('UNIDAD'),1,0,'C',1);
	$pdf->Cell(40,6, utf8_decode('NOMBRE'),1,0,'C',1);
	$pdf->Cell(30,6, utf8_decode('MARCA'),1,0,'C',1);
	$pdf->Cell(50,6, utf8_decode('DESCRIPCIÓN'),1,1,'C',1);
	$pdf->SetFont('Arial','',8);
	while($fila = $consulta->fetch_array())
	{
		$pdf->Cell(10,6, utf8_decode(''),0,0,'C',1);
		$pdf->Cell(25,6,utf8_decode($fila['cantidad']),1,0,'C');
		$pdf->Cell(25,6,utf8_decode($fila['unidad']),1,0,'C');
		$pdf->Cell(40,6,utf8_decode($fila['nombre']),1,0,'C');
		$pdf->Cell(30,6,utf8_decode($fila['marca']),1,0,'C');
		$pdf->Cell(50,6,utf8_decode($fila['descripcion']),1,1,'C');
	}
	$pdf->Ln(6);
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','B',10);
